...
void print_if_directory(int type) {
    switch (type) {
        //case DT_BLK:  printf("b "); break;
        //case DT_CHR:  printf("c "); break;
...
/**
 ...
 @print_lenth длинна до которой надо расширить
 */
...
void ls(const char *dir) {
    ...
    while( (entry=readdir(folder)) )
    {
        ...
        print_if_directory(entry->d_type);
    }
    closedir(folder);
    tab_count--;
}
...
//    opterr=0;
    while ( (rez = getopt(argc,argv,"hf:")) != -1){
        switch (rez){
            case 'h': printf("This is example of list directory\n");
